...
import { ChevronDown, ChevronUp, ExternalLink, BookOpen, Code, GraduationCap, Play, CheckCircle, XCircle, Sparkles, Loader2 } from "lucide-react";
...
export function RoadmapClient({ role }: { role: any }) {
  ...
  const [assessCourse, setAssessCourse] = useState<any>(null);
  ...
  const load = useCallback(async () => {
    ...
    if (!roleId) { setLoading(false); setItems([]); return; }
  ...
  async function generateCourses() {
    ...
    toast("Courses generated!", `${data.saved} resources saved.`);
    load();
  }
  ...
  async function startAssessment(course: any, skillName: string) {
    setAssessLoading(true);
    setAssessCourse(course);
  ...
  async function submitAssessment() {
    ...
    if (passed) {
      await supabase.from("roadmap_courses").update({ completed: true }).eq("id", assessCourse.id);
    }

    setAssessResult({ score, correct, total, passed });
  }
          ...
          {computed.map((x) => {
            ...
            return (
              <Card key={x.skill_id} className="bg-white/5 overflow-hidden">
                ...
                {/* Expanded content — courses & books */}
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div
                    ...
                    >
                      <CardContent className="px-5 pb-5 pt-0 border-t border-white/5">
                        {x.courses.length > 0 ? (
                                ...
                                <div key={ci} className={`rounded-xl border p-4 ${course.completed ? "border-emerald-500/20 bg-emerald-500/5" : "border-white/10 bg-white/5"}`}>
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex flex-col gap-2 shrink-0">
                                      ...
                                      {!course.completed && (
                                        <button onClick={() => startAssessment(course, x.name)} className="rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 p-2 transition text-emerald-300">
                          ...
                          <div className="mt-4 text-sm text-zinc-400 text-center py-4">
      ...
      <AnimatePresence>
        {assessCourse && (
          ...
          >
            ...
            >
              <h3 className="text-xl font-semibold">Assessment: {assessCourse.title}</h3>
              ...
              {assessResult && (
                <div className="mt-6 space-y-5 text-center">
                  ...
                  <Badge tone={assessResult.passed ? "good" : "bad"}>
                    {assessResult.passed ? "Passed! Course completed." : "Not passed. Try again after review."}
                  </Badge>
